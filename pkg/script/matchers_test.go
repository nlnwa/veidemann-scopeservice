	...
	tests := []testdata{
		...
		{name: "isSameHost2",
			...
			want: &scopechecker.ScopeCheckResponse{
				...
				ExcludeReason:   Include.AsInt32(),
				IncludeCheckUri: "http://sub.foo.bar/aa%20bb/cc?foo&jsessionid=1",
				Console:         "",
			}},
		{name: "isSameHostSub1",
...
func Test_isScheme(t *testing.T) {
  ...
  <builtin>: in param
Error: no value with name 'scheme'`,
				},
			}},
		{name: "isScheme6",
			script: "isScheme('http').then(Include).abort()",
		...
		{name: "isScheme7",
			...
			want: &scopechecker.ScopeCheckResponse{
				...
				IncludeCheckUri: "file:///c:/foo/bar/aa%20bb/",
				Console:         "",
			}},
	}
	for _, tt := range tests {
	...
	tests := []testdata{
		{name: "isUrl1",
			...
			want: &scopechecker.ScopeCheckResponse{
				Evaluation:      scopechecker.ScopeCheckResponse_INCLUDE,
				ExcludeReason:   Include.AsInt32(),
				IncludeCheckUri: "http://foo.bar/aa/bb/cc?foo&jsessionid=1",
				Console:         "",
			}},
		{name: "isUrl2",
			...
			want: &scopechecker.ScopeCheckResponse{
				Evaluation:      scopechecker.ScopeCheckResponse_INCLUDE,
				ExcludeReason:   Include.AsInt32(),
				IncludeCheckUri: "http://foo.bar/aa/bb/cc?a=c&a=b&foo&jsessionid=1",
				Console:         "",
			}},
		{name: "isUrl3",
		...
		{name: "isUrl4",
			...
			want: &scopechecker.ScopeCheckResponse{
				Evaluation:      scopechecker.ScopeCheckResponse_EXCLUDE,
				ExcludeReason:   Blocked.AsInt32(),
				IncludeCheckUri: "http://foo.bar/aa/",
				Console:         "",
			}},
	}
	for _, tt := range tests {
...
func verify(t *testing.T, got, want *scopechecker.ScopeCheckResponse) {
	...
	if got.IncludeCheckUri != want.IncludeCheckUri {
